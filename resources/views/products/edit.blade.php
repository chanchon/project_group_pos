        ...
        <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <div class="custom-file">
                    ...
                    <label class="custom-file-label" for="image">เลือกรุปสินค้า</label>
                </div>
                @error('image')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
            ...
            <div class="form-group">
                ...
                <select name="status" class="form-control @error('status') is-invalid @enderror" id="status">
                    <option value="1" {{ old('status', $product->status) === 1 ? 'selected' : ''}}>พร้อมจำหน่าย</option>
                    <option value="0" {{ old('status', $product->status) === 0 ? 'selected' : ''}}>ไม่พร้อมจำหน่าย</option>
                </select>
                @error('status')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
